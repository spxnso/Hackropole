...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python prepare.py <url> <directory> (optional) --overwrite (optional)"
        )
        sys.exit(1)
    ...
    print(
        f"Found challenge: {challenge['title']}, by {author['name']}, difficulty: {difficulty}"
    )
    ...
    if docker_url:
        ...
        subprocess.run(["curl", docker_url, "-o", f"{directory}/docker-compose.yml"])
    else:
